...
export default function RoomGrid({ initialRooms = [] }: RoomGridProps) {
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT' && payload.new) {
            ...
            setSensorReadings(prevReadings => {
              ...
              const updatedReadings = prevReadings.filter(r => r.room_id !== newReading.room_id)
  ...
  const roomStatuses: RoomStatus[] = useMemo(() => {
    return rooms.map(room => {
      const latestReading = sensorReadings.find(reading => reading.room_id === room.id)
  ...
  const filteredAndSortedRoomStatuses = useMemo(() => {
    ...
    if (filters.availability !== 'all') {
      filtered = filtered.filter(rs => rs.availability === filters.availability)
    }
  ...
  const availableFloors = useMemo(() => 
    [...new Set(rooms.map(room => room.floor))], [rooms]
  )
  const availableBuildings = useMemo(() => 
    [...new Set(rooms.map(room => room.building))], [rooms]
  )
