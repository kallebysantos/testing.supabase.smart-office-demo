...
export function FloorplanViewer({ className = '' }: FloorplanViewerProps) {
  ...
  const fetchRooms = async () => {
    try {
      const { data: roomsData, error: roomsError } = await supabase.from('rooms').select('*').limit(4).order('id')
  ...
  return (
    <TooltipProvider>
      <div className={`relative w-full max-w-4xl mx-auto ${className}`}>
        ...
        <div className="mb-2 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">
              {realtimeStatus === 'connected' ? 'Live updates active' : realtimeStatus === 'connecting' ? 'Connecting...' : 'Offline'}
            </span>
          </div>
        </div>
        ...
        <div ref={containerRef} className="relative w-full h-[400px] bg-white rounded-lg overflow-hidden border border-gray-200">
          <Image
          ...
          {rooms.map((room, index) => {
            ...
            return (
              <Tooltip key={room.id}>
                ...
                <TooltipContent side="top" className="bg-white/95 backdrop-blur-sm border-gray-200 p-3 min-w-[250px]">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <h3 className="font-semibold text-gray-900">{room.name}</h3>
                      <Badge variant={getStatusBadgeVariant(status)}>{status === 'over-capacity' ? 'Over Capacity' : status}</Badge>
                    </div>
                    ...
                    {sensor && (
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div className="flex items-center gap-1 text-gray-600">
                          ...
                          <span>
                            {sensor.occupancy}/{room.capacity}
                          </span>
                        </div>
                        <div className="flex items-center gap-1 text-gray-600">
