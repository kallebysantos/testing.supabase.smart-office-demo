...
import { 
  Building2, TrendingUp, AlertTriangle, Clock, Activity, BarChart3, Loader2, 
  Database, Zap, ArrowUp, ArrowDown, Calendar
} from 'lucide-react'
...
export default function AnalyticsPage() {
  ...
  const fetchAnalyticsData = useCallback(async () => {
    ...
    try {
      ...
      const roomUtilization: RoomUtilizationData[] = roomsData?.map(room => {
        const roomSensors = sensorData?.filter(s => s.room_id === room.id) || []
        const roomBookings = bookingsData?.filter(b => b.room_id === room.id) || []
        
        const avgOccupancy = roomSensors.length > 0 
          ? roomSensors.reduce((sum, s) => sum + s.occupancy, 0) / roomSensors.length 
          : 0
        
        const avgUtilization = room.capacity > 0 ? (avgOccupancy / room.capacity) * 100 : 0
        const capacityViolations = roomSensors.filter(s => s.occupancy > room.capacity).length
        
        const avgTemperature = roomSensors.length > 0
          ? roomSensors.reduce((sum, s) => sum + s.temperature, 0) / roomSensors.length
          : 72
          
        const avgAirQuality = roomSensors.length > 0
          ? roomSensors.reduce((sum, s) => sum + s.air_quality, 0) / roomSensors.length
          : 85
          
        const avgNoiseLevel = roomSensors.length > 0
          ? roomSensors.reduce((sum, s) => sum + s.noise_level, 0) / roomSensors.length
          : 45

        // Determine utilization trend (simplified)
        const trend = avgUtilization > 70 ? 'up' : avgUtilization < 30 ? 'down' : 'stable'
      ...
      for (let i = 13; i >= 0; i--) {
        ...
        const dayData = sensorData?.filter(s => 
        ...
        const avgUtilization = dayData.length > 0
          ? dayData.reduce((sum, s) => {
              ...
              return sum + (room?.capacity ? (s.occupancy / room.capacity) * 100 : 0)
            }, 0) / dayData.length
        ...
        timeSeriesData.push({
          ...
          avg_temperature: dayData.length > 0 
            ? Math.round((dayData.reduce((sum, s) => sum + s.temperature, 0) / dayData.length) * 10) / 10
            : 72 + (Math.random() - 0.5) * 4,
          avg_air_quality: dayData.length > 0
            ? Math.round(dayData.reduce((sum, s) => sum + s.air_quality, 0) / dayData.length)
            : 80 + Math.round(Math.random() * 15),
          avg_noise: dayData.length > 0
            ? Math.round(dayData.reduce((sum, s) => sum + s.noise_level, 0) / dayData.length)
      ...
      for (let hour = 8; hour <= 18; hour++) {
        ...
        const hourData = sensorData?.filter(s => {
          const sensorHour = new Date(s.timestamp).getHours()
          return sensorHour === hour
        }) || []
        
        const avgOccupancy = hourData.length > 0
          ? hourData.reduce((sum, s) => sum + s.occupancy, 0) / hourData.length
          : Math.random() * 15 + 5
        
        const hourBookings = bookingsData?.filter(b => {
      ...
      const roomPerformance: RoomPerformanceData[] = roomUtilization.slice(0, 8).map(room => ({
        room_name: room.room_name.length > 15 ? room.room_name.substring(0, 15) + '...' : room.room_name,
        ...
        temperature_variance: Math.max(0, 100 - Math.abs(room.avg_temperature - 72) * 5)
      }))

      const analyticsData: AnalyticsDashboard = {
        roomUtilization: roomUtilization.sort((a, b) => b.avg_utilization - a.avg_utilization),
        timeSeriesData,
        hourlyUsage,
        roomPerformance,
        totalRooms: roomsData?.length || 0,
        totalBookings: bookingsData?.length || 0,
        avgUtilization: Math.round(roomUtilization.reduce((sum, r) => sum + r.avg_utilization, 0) / (roomUtilization.length || 1)),
        criticalIssues: roomUtilization.filter(r => r.capacity_violations > 5 || r.avg_air_quality < 70).length
      }
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <div className="text-sm text-gray-600">
            {icebergEnabled ? 'Loading with Analytics Buckets...' : 'Querying PostgreSQL...'}
          </div>
          {queryTime > 0 && (
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-gray-600 mt-2">
                  Historical room utilization, performance trends, and capacity analysis
                </p>
                <div className="mt-3">
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-sm text-gray-600">
                    Query time: <span className="font-medium">{queryTime}ms</span>
                  </span>
                </div>
                ...
                >
                  <Zap className={`h-4 w-4 ${icebergEnabled ? 'text-white' : 'text-blue-600'}`} />
                  <span>Turn {icebergEnabled ? 'off' : 'on'} Analytics Buckets</span>
                </Button>
              </div>
            </div>
            ...
            {icebergEnabled && (
              <div className="bg-gradient-to-r from-blue-50 to-green-50 border border-blue-200 rounded-lg p-4 mb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-blue-900">Analytics Buckets with Apache Iceberg</h3>
                    <p className="text-sm text-blue-700">
                      Large-scale analytics with open table format. Query performance improved {Math.round(3500/queryTime)}x faster! 
                      Features: Time travel, schema evolution, and bottomless data model.
                    </p>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-bold text-green-600">{Math.round(3500/queryTime)}x</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-gray-500">Conference rooms tracked</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-green-600">{analytics.avgUtilization}%</div>
            ...
            <Card className="col-span-1">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={analytics.roomUtilization.slice(0, 10)} layout="horizontal">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  {analytics.roomUtilization
                    ...
                    .map((room, index) => (
                      <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            ...
                            <div className="flex items-center space-x-4 text-xs text-gray-500">
                              {room.capacity_violations > 3 && (
                                <span className="text-red-600">
                                  🚨 {room.capacity_violations} capacity violations
                                </span>
                              )}
                              {room.avg_air_quality < 75 && (
                                <span className="text-orange-600">
                                  💨 Poor air quality ({room.avg_air_quality}/100)
                                </span>
                              )}
                              {room.avg_utilization > 90 && (
                        ...
                        <div className="text-right">
                          <div className="text-sm font-medium">{room.avg_utilization}%</div>
                  ...
                  {analytics.roomUtilization.filter(room => 
                    room.capacity_violations > 3 || 
                    room.avg_air_quality < 75 || 
                    room.avg_utilization > 90 ||
                    room.avg_utilization < 15
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200">
                      ...
                      <th className="text-center py-3 px-4 font-medium text-gray-600">Avg Occupancy</th>
                      <th className="text-center py-3 px-4 font-medium text-gray-600">Violations</th>
                      <th className="text-center py-3 px-4 font-medium text-gray-600">Temp</th>
                      <th className="text-center py-3 px-4 font-medium text-gray-600">Air Quality</th>
                      <th className="text-center py-3 px-4 font-medium text-gray-600">Noise</th>
                      <th className="text-center py-3 px-4 font-medium text-gray-600">Trend</th>
                    </tr>
                  </thead>
                  <tbody>
                    {analytics.roomUtilization.slice(0, 12).map((room, index) => (
                        ...
                        <td className="py-3 px-4 text-center">
                          <div className="flex items-center justify-center space-x-2">
                            <div className={`w-2 h-2 rounded-full ${
                              room.avg_utilization > 80 ? 'bg-red-500' :
                              room.avg_utilization > 50 ? 'bg-yellow-500' : 'bg-green-500'
                            }`}></div>
                            <span className="font-medium">{room.avg_utilization}%</span>
                          </div>
                        </td>
                        <td className="py-3 px-4 text-center text-gray-700">{room.total_bookings}</td>
                        <td className="py-3 px-4 text-center text-gray-700">{room.avg_occupancy}</td>
                        <td className="py-3 px-4 text-center">
                          <span className={room.capacity_violations > 0 ? 'text-red-600 font-medium' : 'text-gray-700'}>
                            {room.capacity_violations}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center text-gray-700">{room.avg_temperature}°F</td>
                        <td className="py-3 px-4 text-center">
                          <span className={room.avg_air_quality < 75 ? 'text-orange-600 font-medium' : 'text-gray-700'}>
                            {room.avg_air_quality}/100
                          </span>
                        </td>
                        <td className="py-3 px-4 text-center text-gray-700">{room.avg_noise_level} dB</td>
                        <td className="py-3 px-4 text-center">
                          {room.utilization_trend === 'up' && <ArrowUp className="h-4 w-4 text-green-600 mx-auto" />}
                          {room.utilization_trend === 'down' && <ArrowDown className="h-4 w-4 text-red-600 mx-auto" />}
                          {room.utilization_trend === 'stable' && <div className="w-4 h-0.5 bg-gray-400 mx-auto"></div>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
