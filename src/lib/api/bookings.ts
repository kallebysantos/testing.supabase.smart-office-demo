...
export class BookingsApi extends ApiClient {
  ...
  async getBookings(): Promise<ApiResponse<BookingWithRoomData[]>> {
    return this.handleResponse(async () => {
      ...
      const bookingsWithRoom = data?.map(booking => ({
        ...
        status: this.determineBookingStatus(booking.start_time, booking.end_time)
      })) || []
   ...
   * Fetch booking by ID
   */
  async getBookingById(id: BookingId): Promise<ApiResponse<BookingWithRoomData>> {
    return this.handleResponse(async () => {
      const { data, error } = await supabase
        .from('room_bookings')
        .select(`
   ...
   */
  async getBookingsWithSensorData(): Promise<ApiResponse<BookingWithSensorData[]>> {
    try {
      ...
      const bookingsWithSensor: BookingWithSensorData[] = bookingsResponse.data.map(booking => {
   ...
   */
  private determineBookingStatus(startTime: string, endTime: string): BookingStatus {
    const now = new Date()
