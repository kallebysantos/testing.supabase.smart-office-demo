...
const FIRST_NAMES = [
  // Harry Potter
  'Hermione', 'Luna', 'Ginny', 'Lily', 'Molly', 'Minerva', 'Nymphadora', 'Bellatrix',
  'Harry', 'Ron', 'Neville', 'Draco', 'Severus', 'Albus', 'Sirius', 'Remus',
  'Arthur', 'Fred', 'George', 'Percy', 'Bill', 'Charlie', 'Cedric',
  
  // Star Wars
  'Leia', 'Padmé', 'Rey', 'Ahsoka', 'Jyn', 'Mon', 'Hera',
  'Luke', 'Han', 'Obi-Wan', 'Anakin', 'Poe', 'Finn', 'Lando',
  
  // Lord of the Rings
  'Arwen', 'Galadriel', 'Eowyn', 'Tauriel',
  'Aragorn', 'Legolas', 'Gimli', 'Boromir', 'Faramir', 'Samwise', 'Frodo'
]

const LAST_NAMES = [
  // Harry Potter
  'Granger', 'Potter', 'Weasley', 'Lovegood', 'Longbottom', 'Malfoy',
  'Snape', 'Dumbledore', 'Black', 'Lupin', 'McGonagall', 'Diggory',
  
  // Star Wars  
  'Organa', 'Amidala', 'Skywalker', 'Solo', 'Kenobi', 'Tano',
  'Erso', 'Mothma', 'Calrissian', 'Dameron', 'Andor',
  
  // Lord of the Rings
  'Evenstar', 'Greenleaf', 'Baggins', 'Gamgee', 'Brandybuck',
  'Took', 'Elessar', 'Undómiel'
]

const DEPARTMENTS = [
  'Corporate Law', 'Litigation', 'Real Estate', 'Employment Law',
  'Intellectual Property', 'Tax Law', 'Family Law', 'Criminal Defense',
  'Immigration', 'Environmental Law', 'Healthcare Law', 'Securities',
  'Mergers & Acquisitions', 'Banking & Finance', 'Insurance Defense',
  'Personal Injury', 'Estate Planning', 'Compliance'
]
...
async function generateUsers() {
  ...
  try {
    ...
    console.log('✅ Supabase connection successful, existing users:', testData?.length || 0)
    ...
    if (deleteProfileError) {
      console.log('⚠️  Could not clear user profiles:', deleteProfileError.message)
    } else {
    ...
    const facilitiesCount = Math.floor(TOTAL_USERS * ROLE_DISTRIBUTION.facilities)
    const employeeCount = TOTAL_USERS - adminCount - facilitiesCount

    console.log(`📊 Generating: ${adminCount} admin, ${facilitiesCount} facilities, ${employeeCount} employees`)
    ...
    for (let i = 0; i < TOTAL_USERS; i++) {
      ...
      let email = `${firstName.toLowerCase().replace(/[^a-z]/g, '')}.${lastName.toLowerCase().replace(/[^a-z]/g, '')}@deweycheathamhowe.com`
      let counter = 1
      while (usedEmails.has(email)) {
        email = `${firstName.toLowerCase().replace(/[^a-z]/g, '')}.${lastName.toLowerCase().replace(/[^a-z]/g, '')}${counter}@deweycheathamhowe.com`
        counter++
      }
      usedEmails.add(email)

      const role = roleQueue[i] as 'admin' | 'facilities' | 'employee'
      const department = role === 'facilities' ? 'Facilities Management' :
    ...
    for (let i = 0; i < usersToInsert.length; i++) {
      ...
      console.log(`🔄 Creating auth user ${i + 1}/${usersToInsert.length}: ${userData.full_name}`)
      ...
      try {
        ...
        if (authError) {
          console.error(`❌ Error creating auth user ${i + 1} (${userData.full_name}):`, authError.message)
          continue
        }
        ...
        console.log(`✅ Created auth user: ${userData.full_name} (${authUser.user.id})`)
        ...
        if (profileError) {
          console.error(`❌ Error updating user profile ${i + 1} (${userData.full_name}):`, profileError.message)
          ...
          console.log('❌ Stopping due to profile update error for investigation')
          break
        }

        insertedCount++
        console.log(`✅ SUCCESS ${i + 1}/${usersToInsert.length}: ${userData.full_name} (${userData.role}) - Auth: ${authUser.user.id}`)
