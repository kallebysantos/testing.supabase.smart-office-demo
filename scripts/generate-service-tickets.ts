...
[
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a facilities management assistant helping to generate realistic service tickets for a law firm office building. Focus on authentic, everyday issues that facilities teams encounter.'
            },
            {
              role: 'user',
              content: prompt
    ...
    const fallbackTickets = [
      {
        ...
        description: "All whiteboard markers in Conference Room 3A are dried out and unusable. Meeting scheduled for 2 PM today needs working markers for presentation.",
        ticket_type: "maintenance",
        severity: "low"
      },
      {
        title: "Kitchen Area - Coffee machine needs descaling",
        description: "Main coffee machine in 4th floor kitchen is producing bitter coffee and needs descaling. Water flow is also slower than usual.",
        ticket_type: "maintenance",
        severity: "medium"
      },
      {
        title: "Reception - Plants need watering",
        description: "Several plants in the main reception area are wilting and need immediate watering. Some leaves are turning brown.",
        ticket_type: "environmental",
        severity: "low"
      },
      {
        title: "Floor 4 - Temperature too cold in hallway",
        description: "Hallway on 4th floor near elevators is uncomfortably cold. Staff are complaining about the temperature being too low.",
        ticket_type: "environmental",
        severity: "medium"
      },
      {
        title: "Boardroom - Projector screen stuck halfway",
        description: "Electric projector screen in main boardroom is stuck in the halfway position and won't retract or extend fully.",
        ticket_type: "maintenance",
        severity: "medium"
      },
      {
        title: "Break Room - Refrigerator making loud noise",
        description: "Refrigerator in 3rd floor break room is making a loud humming noise that's disturbing nearby offices.",
        ticket_type: "maintenance",
        severity: "medium"
      },
      {
        title: "Lobby - Automatic door sensor not working",
        description: "Automatic door sensor on main entrance is not detecting people approaching. Door requires manual operation.",
        ticket_type: "maintenance",
        severity: "high"
      },
      {
        title: "Restroom - Paper towel dispenser empty",
        description: "Paper towel dispenser in men's restroom on 2nd floor is empty and needs refilling.",
        ticket_type: "maintenance",
        severity: "low"
      },
      {
        title: "Conference Room B - Air conditioning not working",
        description: "Air conditioning unit in Conference Room B is not cooling properly. Room temperature is 78°F and rising.",
        ticket_type: "environmental",
        severity: "high"
      },
      {
        title: "Copy Room - Printer jam needs clearing",
        description: "Main printer in copy room has a paper jam that needs to be cleared. Multiple users unable to print.",
        ticket_type: "maintenance",
        severity: "medium"
      }
    ]
   ...
   */
  private assignStatusAndPriority(severity: string): { status: string, priority: number } {
  ...
  private generateTicketId(): string {
    return `INC${String(Date.now() + Math.floor(Math.random() * 1000)).slice(-7)}`
  }

  /**
   * Get random room for ticket assignment
   */
  private getRandomRoom(rooms: Room[]): Room {
   ...
   */
  private generateAssignmentData(status: string): any {
    ...
    if (status === 'assigned' || status === 'resolved') {
      data.assigned_to = facilitiesTeam[Math.floor(Math.random() * facilitiesTeam.length)]
      data.assigned_at = new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString() // Within last week
    }

    if (status === 'resolved') {
      data.resolved_at = new Date(Date.now() - Math.random() * 3 * 24 * 60 * 60 * 1000).toISOString() // Within last 3 days
      ...
      data.resolution_notes = resolutionNotes[Math.floor(Math.random() * resolutionNotes.length)]
    }
...
async function generateServiceTickets() {
  ...
  try {
    ...
    const totalTickets = ticketCounts[Math.floor(Math.random() * ticketCounts.length)]
    ...
    const tickets: ServiceTicket[] = ticketData.map((ticket: any) => {
      ...
      const { status, priority } = generator.assignStatusAndPriority(ticket.severity)
    ...
    for (let i = 0; i < tickets.length; i += batchSize) {
      ...
      if (insertError) {
        ...
        console.log(`✅ Inserted ${batch.length} tickets (${totalInserted} total)`)
      }
    }
    ...
    console.log(`   • Rooms with tickets: ${new Set(tickets.map(t => t.room_id)).size}`)
    console.log('')
