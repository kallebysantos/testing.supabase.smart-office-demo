...
class SmartSensorDataGenerator {
  ...
  generateOccupancy(
    ...
    const currentBooking = this.isRoomCurrentlyBooked(
      room.id,
      bookings,
      currentTime
    );
    ...
    if (currentBooking) {
      ...
      if (isNoShow) {
        ...
        const bookingAttendees =
          currentBooking.attendee_count || Math.ceil(room.capacity * 0.5);
        const variance = Math.floor(bookingAttendees * 0.3); // ±30% variance
        const randomVariance = Math.floor((Math.random() - 0.5) * 2 * variance);

        targetOccupancy = Math.max(
          1,
          Math.min(room.capacity, bookingAttendees + randomVariance)
        );
      }
    } else {
      ...
      if (capacityFactor < 0.7) {
        ...
        targetOccupancy = Math.floor(
          Math.random() * Math.min(3, room.capacity * 0.2)
        );
    ...
    if (previousOccupancy !== undefined) {
      ...
      const consistentChange = Math.min(
        Math.abs(change),
        Math.abs(targetOccupancy - previousOccupancy)
      );
  ...
  generateTemperature(
    ...
    if (previousTemp !== undefined) {
      ...
      const change = Math.max(
        -maxTempChange,
        Math.min(maxTempChange, targetTemperature - previousTemp)
      );
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(
      "⏱️  Running 45-second sensor simulation with 3-second intervals..."
    );
    ...
    while (Date.now() - startTime < simulationDuration) {
      ...
      if (insertError) {
        ...
        console.log(
          `✅ Inserted ${readings.length} sensor readings (${totalReadings} total)`
        );
      }
    ...
    // Create a capacity violation alert by selecting one random room and setting it 5 people over capacity
    const randomRoom = rooms[Math.floor(Math.random() * rooms.length)];
    const violationOccupancy = randomRoom.capacity + 5;

    console.log(
      `🚨 Creating capacity violation: ${randomRoom.name} (capacity: ${randomRoom.capacity}, occupancy: ${violationOccupancy})`
    );

    const violationReading: SensorReading = {
      room_id: randomRoom.id,
      occupancy: violationOccupancy,
      temperature: generator.generateTemperature(
        randomRoom,
        violationOccupancy
      ),
      noise_level: generator.generateNoiseLevel(randomRoom, violationOccupancy),
      air_quality: generator.generateAirQuality(randomRoom, violationOccupancy),
    };

    // Insert the capacity violation reading
    const { error: violationError } = await supabase
      .from("sensor_readings")
      .insert([violationReading]);

    if (violationError) {
      console.error(
        "❌ Error inserting capacity violation reading:",
        violationError
      );
    } else {
      console.log(`✅ Capacity violation created for ${randomRoom.name}`);
      totalReadings += 1;
    }
    ...
    console.log(
      `🎉 Simulation completed: ${totalReadings} readings over ${actualDuration.toFixed(
        1
      )} seconds`
    );
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message:
          "Intelligent sensor data simulation completed with capacity violation",
        stats: {
          totalReadings,
          ...
          capacityViolation: {
            ...
            violationOccupancy: violationOccupancy,
            overCapacityBy: 5,
          },
        },
      }),
